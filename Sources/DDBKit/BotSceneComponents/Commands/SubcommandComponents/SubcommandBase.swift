...
public struct SubcommandBase: BaseCommand, IdentifiableCommand, _ExtensibleCommand {
  ...
  public func trigger(_ i: Interaction, _ instance: BotInstance) async throws {
    ...
    guard let (command, options) = try? self.findChild(i)
      ...
      return GS.s.logger.debug(
        "[\(self.baseInfo.name)] SubcommandBase triggered and failed to find subcommand.")
    }
  ...
  public func autocompletion(
    _ i: Interaction, cmd: Interaction.ApplicationCommand,
    opt: Interaction.ApplicationCommand.Option, client: any DiscordClient
  ) async {
    ...
    guard let (command, _) = try? self.findChild(i) else {
      GS.s.logger.debug(
        "[\(self.baseInfo.name)] Autocompletion was called and no handler found for option path.")
      return
    }
...
extension SubcommandBase {
  func findChild(_ i: Interaction) throws -> (Subcommand, [Interaction.ApplicationCommand.Option]?)
  {
    ...
    guard let options, !options.isEmpty else {
      GS.s.logger.debug(
        "[\(self.baseInfo.name)] SubcommandBase triggered with no options, skipped eval.")
