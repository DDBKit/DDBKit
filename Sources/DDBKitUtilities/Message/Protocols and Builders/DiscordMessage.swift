...
public struct Message: Sendable {
    ...
    self.components.rows.reduce([Interaction.ActionRow]()) { partialResult, row in
      ...
      partialResult.append(
        Interaction.ActionRow(
          components: (row as! _ActionRowProtocol).components.map {
            ($0 as! _MessageComponentsActionRowComponent).component
          }))
  ...
  var _attachments: [Payloads.Attachment]? {
    self.attachments.enumerated().compactMap {
...
extension Message {
  ...
  public func tts(_ enabled: Bool = true) -> Self {
    var copy = self
    copy._tts = true
    return copy
  }
  ...
  public func ephemeral(_ enabled: Bool = true) -> Self {
    var copy = self
    if enabled {
  ...
  public func flags(_ flags: IntBitField<DiscordChannel.Message.Flag>) -> Self {
    var copy = self
    copy._flags = flags
    return copy
  }
