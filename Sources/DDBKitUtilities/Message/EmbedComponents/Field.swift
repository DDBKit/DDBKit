...
public struct Field: MessageEmbedComponent {
  ...
  public init(_ name: @autoclosure () -> MessageContentComponent, _ value: @autoclosure () -> MessageContentComponent) {
    self.field = .init(
  ...
  public init(
    ...
    self.field = .init(
      name: name().textualRepresentation,
      value: value().reduce("") { $0 + $1.textualRepresentation }.trimmingCharacters(in: .whitespacesAndNewlines),
      inline: nil
    )
  }
}
...
public extension Field {
  func inline(_ bool: Bool = true) -> Self {
    var copy = self
