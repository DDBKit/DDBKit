...
public actor BucketRatelimiting: DDBKitExtension {
  ...
  ///   - config: Optional custom configuration for this command. If nil, the default configuration is used.
  /// - Throws: `Error.ratelimited` with next available use time if rate limited
  public func ratelimitChecked(
    guildId: GuildSnowflake?,
    userId: UserSnowflake,
    command: String,
    ...
    if var bucket = buckets[bucketKey] {
      ...
      var bucket = Bucket(config: config ?? defaultConfig)
      bucket.recordUse()
      buckets[bucketKey] = bucket
    }
  }
  ...
  private struct Bucket {
    ...
    mutating func canUse() -> (allowed: Bool, nextUse: Date) {
      let now = Date()
      uses = uses.filter {
        now.timeIntervalSince($0.timestamp) < config.timeWindow
      }
