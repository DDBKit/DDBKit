...
extension Database {
  ...
  @discardableResult
  public func transaction<T: DatabaseModel>(
    _ req: FetchRequest<T>, transaction: @escaping (T?) async -> T?
  ) async -> T? {
    let id = req.datahash
  ...
  public func availableItems<T: DatabaseModel>(around req: FetchRequest<T>) -> [FetchRequest<T>] {
    ...
    let files =
      (try? FileManager.default.contentsOfDirectory(
        at: path, includingPropertiesForKeys: [],
  ...
  public func findKeys<T: DatabaseModel>(
    around req: FetchRequest<T>, filter: @escaping @Sendable (T?) -> Bool
  ) async throws -> [String] {
  ...
  public func findItems<T: DatabaseModel>(
    around req: FetchRequest<T>, filter: @escaping @Sendable (T?) -> Bool
  ) async throws -> [FetchRequest<T>] {
    let keys: [String] = availableKeys(around: req)
    var filterPassKeys = [String]()
  ...
  public func sort<T: DatabaseModel>(
    on req: FetchRequest<T>, compare: @escaping @Sendable (T?, T?) -> Bool
  ) async throws -> [String] {
