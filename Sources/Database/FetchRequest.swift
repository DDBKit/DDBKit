...
extension Database {
  ...
  public struct FetchRequest<Model: DatabaseModel>: Sendable {
    ...
    public static func requestFor(user: UserSnowflake, ofType type: Model.Type) -> FetchRequest<
      Model
    > {
    ...
    public static func requestFor(guild: GuildSnowflake, ofType type: Model.Type) -> FetchRequest<
      Model
    > {
    ...
    public static func requestFor(
      member: UserSnowflake, in guild: GuildSnowflake, ofType type: Model.Type
    ) -> FetchRequest<Model> {
      let url = DBPaths.Guilds.url
        .appendingPathComponent(guild.rawValue, isDirectory: true)
    ...
    public static func requestFor(
      channel: ChannelSnowflake, in guild: GuildSnowflake, ofType type: Model.Type
    ) -> FetchRequest<Model> {
      let url = DBPaths.Guilds.url
        .appendingPathComponent(guild.rawValue, isDirectory: true)
    ...
    func _rerouted(to key: String) -> Self {
      ...
      copy.datapath = copy.datapath.deletingLastPathComponent().deletingLastPathComponent()
        .appending(components: key, ext)
      return copy
    }
