		...
		Group {
			...
			SubcommandBase("dict") {
				Subcommand("apple") { i in
					...
					let dict = "Apple"
					let q = try (i.options ?? []).requireOption(named: "query").requireString()
					...
					try await i.respond(with: String((result.data(.text) ?? "idk bro").prefix(2000)))
				}
				.addingOptions {
				...
				Subcommand("search") { i in
					let dict = try i.options?.requireOption(named: "dictionary").requireString()
					let q = try (i.options ?? []).requireOption(named: "query").requireString()
			...
			SubcommandBase("math") {
				...
				Subcommand("eval") { i in
					let expr = try (i.options ?? []).requireOption(named: "expr").requireString()
					let first = Date.now
					...
					try await i.respond {
						Message {
							MessageEmbed {
								...
								Footer("\(result.succeed == 1 ? "" : "Evaluation failed • ")Took \(formatted)")
							}
					...
					try await i.respond {
						Message {
							MessageEmbed {
								...
								Footer("\(!result.isFailedResult ? "" : "Evaluation failed • ")Took \(formatted)")
							}
