...
public class Fuse: @unchecked Sendable {
  ...
  private func _search(_ pattern: Pattern, in aString: String) -> (
    ...
    while index != nil {
      ...
      let score = FuseUtilities.calculateScore(
        pattern.len,
        e: 0,
        x: i,
        loc: location,
    ...
    for i in 0..<pattern.len {
      ...
      while binMin < binMid {
        if FuseUtilities.calculateScore(
          pattern.len, e: i, x: location, loc: location + binMid, distance: distance) <= threshold
        {
          binMin = binMid
        } else {
          binMax = binMid
...
extension Fuse {
  ...
  public func search(
    ...
    completion: @Sendable @escaping ([SearchResult]) -> Void
  ) {
    let pattern = self.createPattern(from: text)
  ...
  public func search(
    ...
    completion: @Sendable @escaping ([FusableSearchResult]) -> Void
  ) {
    let pattern = self.createPattern(from: text)
    ...
    stride(from: 0, to: count, by: chunkSize).forEach { offset in
      ...
      self.searchQueue.async {
        ...
        for (index, item) in chunk.enumerated() {
          ...
          var propertyResults = [
            (key: String, score: Double, ranges: [CountableClosedRange<Int>])
          ]()
